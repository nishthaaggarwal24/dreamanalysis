...
def analyze_dream_text(text):
    ...
    if any(w in text_lower for w in ['flying', 'happy', 'sun', 'peace', 'joy', 'friend', 'beautiful', 'light']):
        emotion = 'Joy'
        sentiment = 'Positive'
        score = round(random.uniform(0.6, 0.95), 2)
        cluster = 'Flight & Escape' if 'flying' in text_lower else 'Relational Connection'
        anxiety = 'Identity Transformation'
    elif any(w in text_lower for w in ['chase', 'monster', 'fear', 'dark', 'falling', 'fail', 'exam', 'stuck', 'late']):
        emotion = 'Fear'
        sentiment = 'Negative'
        score = round(random.uniform(-0.95, -0.4), 2)
        cluster = 'Performance Anxiety' if 'exam' in text_lower or 'late' in text_lower else 'Flight & Escape'
        anxiety = 'Academic Pressure' if 'exam' in text_lower else 'Loss of Control'
    elif any(w in text_lower for w in ['crying', 'lost', 'alone', 'empty', 'sad', 'pain']):
        emotion = 'Sadness'
        sentiment = 'Negative'
        score = round(random.uniform(-0.8, -0.3), 2)
        cluster = 'Existential Transformation'
        anxiety = 'Social Isolation'
    else:
        emotion = 'Surprise' if 'door' in text_lower or 'secret' in text_lower else 'Neutral'
        sentiment = 'Neutral' if emotion == 'Neutral' else 'Positive'
        score = round(random.uniform(-0.1, 0.4), 2)
        cluster = 'Existential Transformation'
        anxiety = 'Career Uncertainty'
    
    symbols = []
    symbol_candidates = ['flying', 'ocean', 'monster', 'forest', 'room', 'crying', 'garden', 'family', 'falling', 'exams', 'teeth', 'car', 'brakes', 'school', 'water']
